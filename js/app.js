...
function renderTodoList(todoListEl, todos) {
  ...
  Array.from(todoListEl.children).forEach((todoList) => {
    todoList.innerHTML = createTodoListHTML(
      todos.filter((todo) => +todoList.dataset.status === todo.status)
    ).join("");
  });
}
...
function createTodoHTML(todo) {
  return `<div class="card todo ${getStatusFromCode(
    todo.status
  )} mb-3" data-id="${todo.id}">
    ...
    <div class="card-body">
      ...
      ${
        todo.status !== 3
          ? `<button class="btn btn-primary btn-next">Change status to "${getStatusFromCode(
              todo.status + 1
            )}"</button>`
          : '<button class="btn btn-danger btn-remove"><i class="bi bi-trash-fill"></i></button>'
      }
    </div>
    <div class="card-footer d-flex justify-content-between">
        <small><i class="bi bi-file-earmark-plus-fill"></i> ${dateFormatter.format(
          todo.createdAt
        )}</small>
        ${
          dateFormatter.format(todo.updatedAt)
            ? `<small><i class="bi bi-check-circle-fill"></i> ${dateFormatter.format(
                todo.updatedAt
              )}</small>`
            : ""
        }
